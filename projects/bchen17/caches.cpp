...
int main(int argc, char *argv[]) {
    ...
    while(infile >> std::hex >> type >> addr ) {
        ...
        two_level_cache_wb_run(cache6c, marked6c, cache6d, marked6d, dirtyb, addr, &(counters[16]), &(counters[17]), &(access_counter[4]), 5, 5, 4, 0, 6, 10, type);
    ...
    for(int i=0; i<4;i++){
        outfile << counters[i] << "," << access_counter[0]  << "; ";
    }
    outfile << endl;
    for(int i=0; i<4;i++){
        outfile << counters[4+i] << "," << access_counter[0]  << "; ";
    }
    outfile << endl;

    for(int i=0; i<2;i++){
        outfile << counters[8+i] << "," << access_counter[0]  << "; ";
    }
    outfile << endl;

    outfile << counters[10] << "," << access_counter[0]  << "; ";
     outfile << counters[11] << "," << access_counter[1]  << "; ";
     outfile << endl;
     outfile << counters[12] << "," << access_counter[0]  << "; ";
     outfile << counters[13] << "," << access_counter[2]  << "; ";
     double k = 0;
    ...
    outfile << k/1024;
    outfile << endl;
    outfile << counters[14] << "," << access_counter[0]  << "; ";
    outfile << counters[15] << "," << access_counter[2]  << "; ";
    k=0;
    for(int i =0;i<1024;i++){
        if(cache6b[i] != 0){
            k++;
    ...
    outfile << std::setprecision(2) << k/1024;
     outfile << endl;
     outfile << counters[16] << "," << access_counter[0]  << "; ";
     outfile << counters[17] << "," << access_counter[2]  << "; ";
     for(int i =0;i<1024;i++){
        if(cache6d[i] != 0){
            k++;
        }
...
void two_level_cache_wb_run(unsigned int *arr1, int *marked_arr1, unsigned int *arr2, int *marked_arr2, int *dirty, unsigned int address, int *counter1, int *counter2, int *access_counter, int target_bits1, int offset_bits1, int set_size1, int target_bits2, int offset_bits2, int set_size2, string type){
    ...
    if(hit < set_size1){
        ...
        unsigned int block3 = (arr1[index1*set_size1 + lru]);
        //block3 = block1;
        arr1[index1*set_size1 + lru] = address ;

        //arr1[index1*set_size1 + lru] = block1;

        //arr1[index1*set_size1 + lru] = block1;
        //block1 = get_block(address, target_bits1, offset_bits1);
        ...
        if(dirty[index1*set_size1 + lru] == 1){
            ...
            unsigned int index3 = block3; 
            if(target_bits2!=0){
                index3 = block3 << (32-target_bits2) >> (32-target_bits2);
            } 
...
int i = 0;
    ...
    if(hit < set_size1){
        ...
        //get the current block in L1

        arr1[index1*set_size1 + lru] = address >> offset_bits1;
        //make arr1 the fetched block
